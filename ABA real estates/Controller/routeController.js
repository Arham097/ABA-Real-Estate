...
exports.getLocationDetails = async (req,res)=>{
    let data = [];
    const sql = "SELECT DISTINCT CONCAT(location,', ',city) AS location FROM housesdataset"
...
exports.details = (req,res)=>{
    conn.query('SELECT * FROM  housesdataset',(result)=>{
        if(err){
            console.error(err);
...
exports.featuredCards = (req,res)=>{
    conn.query('SELECT * FROM housesdataset limit 10',(err,result)=>{
...
exports.fetchForSaleCards = (req,res)=>{
    conn.query('SELECT * FROM housesdataset WHERE purpose = "For Sale" LIMIT 10',(err,result)=>{
        result = result.map(obj => ({ ...obj, price: convertToPakistaniCounting(obj.price)}));
        res.status(200).json({
            status:'success',
            result
...
exports.fetchForRentCards = (req,res)=>{
    conn.query('SELECT * FROM housesdataset WHERE purpose = "For Rent" LIMIT 10',(err,result)=>{
        result = result.map(obj => ({ ...obj, price: convertToPakistaniCounting(obj.price)}));
        res.status(200).json({
            status:'success',
            result
...
exports.searchProperties = async(req,res)=>{
    ...
    const sql = `SELECT * FROM housesdataset WHERE (location=? AND city=? AND property_type=?  AND (price>=? AND price<=?) AND (area>=? AND area<=?)) LIMIT ? OFFSET ? `;
